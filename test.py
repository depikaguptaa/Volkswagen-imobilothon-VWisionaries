...
def get_variant_specs(variant_url):
    ...
    if price_tag:
        ...
        # Clean the price by removing everything after the first occurrence of "Lakh" or "Lakh*" (or similar patterns)
        price = re.sub(r"(\d+(\.\d+)?\s*(Lakh|Cr|₹)[^A-Za-z0-9]+).*", r"\1", price)
    else:
...
# Function to Write Specs Data to TXT File with Enhanced Debugging
